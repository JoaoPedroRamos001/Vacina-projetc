...
db.serialize(() => {
  db.run("CREATE TABLE IF NOT EXISTS formularios (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, email TEXT, mensagem TEXT, cidade TEXT, telefone TEXT)");
});
...
app.post('/submit', (req, res) => {
  const { nome, email, mensagem, cidade, telefone  } = req.body;

  // Exibir informações no console
  console.log(`Nome: ${nome}, Email: ${email}, Mensagem: ${mensagem}, Cidade: ${cidade}, Telefone: ${telefone},`);

  // Inserir dados no banco de dados
  const stmt = db.prepare("INSERT INTO formularios (nome, email, mensagem, cidade, telefone) VALUES (?, ?, ?, ?, ?)");
  stmt.run(nome, email, mensagem, cidade, telefone);
  stmt.finalize();
